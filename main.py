...
def set_page(page_id: int, **kwargs):
    ...
    elif page_id == PAGE_SETTING:
        music.stop(fade_out_ms=400)
    elif page_id == PAGE_DEV:
        music.stop(fade_out_ms=400)
    elif page != page_id:
        music.stop(fade_out_ms=400)
    page = page_id
...
def _play_world_music(world):
    """根据地图配置播放背景音乐（支持合成音乐和文件音乐）。"""
    music_name = getattr(world, 'music', '')
    if music_name and music_name in ("home", "adventure", "danger", "victory"):
        # 合成音乐曲目
        music.play(music_name, fade_in_ms=1200)
    elif music_name:
        # 文件音乐（向后兼容）
        import os
        ...
        # 默认：冒险主题
        music.play("adventure", fade_in_ms=1200)
...
def handle_setting_input(event):
    ...
    if event.key == pygame.K_UP:
        ...
        if _setting_selected_row == 0:
            ...
            music.set_volume(sfx.get_music_volume())
            sfx.play_click()
...
while running:
    ...
    if page == PAGE_DEV:
        ...
        if _game_win and not _win_played:
            sfx.play_win()
            music.play("victory", fade_in_ms=300, loops=0)
            _win_played = True

        # 低血量自动切换危险主题
        if player1.alive and not _game_win and not _game_over:
            hp_ratio = player1.hp / max(1, player1.hp_max)
            if hp_ratio < 0.25 and music.get_current_track() != "danger":
                music.play("danger", fade_in_ms=500)
            elif hp_ratio >= 0.25 and music.get_current_track() == "danger":
                # 恢复 → 回到地图音乐
                _play_world_music(_current_map)
